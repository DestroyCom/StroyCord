...
npm run lint       # ESLint --fix on .js/.ts files
npm run start      # Run compiled dist/Bot.js (production)
npm run knip       # Detect unused exports/files
```

There are no automated tests. The bot requires a running MongoDB instance and valid `.env` to start.
...
Copy `.env.dist` to `.env`. Required vars: `DISCORD_TOKEN`, `DISCORD_CLIENT_ID`, `DATABASE_CONNECTION_STRING`, `DATABASE_USER`, `DATABASE_PASSWORD`. Optional: `PREFIX` (default `&`), `LANGUAGE` (default `en-US`), `DETECT_FROM_ALL_MESSAGES` (default `false` — if `true`, bot responds to YouTube URLs in any message without prefix).
